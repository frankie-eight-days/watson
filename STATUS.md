...
- [x] Observability alert banner + podcast player — slim global banner raises on any `error` event and on folded cost > $5 (COST_ALERT_USD), click → jumps to that agent's console; Conference shows a themed <audio> player + download when a report/podcast artifact (body=MP3 URL) exists, nothing otherwise. Verified all 5 routes render live in headless Chrome. Deployed.
